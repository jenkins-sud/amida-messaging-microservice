...
sequelize.query('ALTER TABLE "UserThreads" ADD COLUMN IF NOT EXISTS "isLog" boolean DEFAULT false NOT NULL; ALTER TABLE "Threads" ADD COLUMN IF NOT EXISTS "lastMessageId" INTEGER; UPDATE "Threads" T1 SET "lastMessageId" = T2."MessageId" FROM ( SELECT max(id) "MessageId", "ThreadId" FROM "Messages" Group By "ThreadId" ) T2 WHERE T1."id" = T2."ThreadId" and "lastMessageId" is null;');
